# 🎭 Discord Undercover Bot

บอท Discord สำหรับเล่นเกม **Undercover** (ลับหรือเปล่า) ภาษาไทย

## ความต้องการ

- **Node.js 18+** (discord.js v14 ต้องการ Node 18 ขึ้นไป)

## การติดตั้ง

```bash
cd discord-undercover-bot
npm install
```

## การตั้งค่า

1. สร้างไฟล์ `.env` จาก template:
```bash
cp .env.example .env
```

2. ดึง Bot Token จาก [Discord Developer Portal](https://discord.com/developers/applications):
   - สร้าง Application
   - ไปที่ Bot > Reset Token และคัดลอก Token
   - เปิด **Message Content Intent** และ **Server Members Intent**
   - เชิญบอทเข้าเซิร์ฟเวอร์ด้วย OAuth2 URL (Scope: bot, Permissions: Send Messages, Read Message History, Manage Messages)

3. ใส่ Token ในไฟล์ `.env`:
```
DISCORD_TOKEN=your_bot_token_here
```

## รันบอท

```bash
npm start
```

## คำสั่ง

| คำสั่ง | คำอธิบาย |
|--------|----------|
| `!uc help` | แสดงวิธีเล่นและคำสั่งทั้งหมด |
| `!uc create` | สร้างห้องเกม (ผู้สร้างเป็น Host) |
| `!uc join` | เข้าร่วมเกม |
| `!uc leave` | ออกจากห้อง (ก่อนเริ่มเกม) |
| `!uc start` | เริ่มเกม (Host เท่านั้น) |
| `!uc word` | ดูคำของตัวเองอีกครั้ง (ส่งทาง DM) |
| `!uc next` | ข้ามไปขั้นตอนโหวต (Host, หลังทุกคนอธิบายครบ) |
| `!uc end` | จบเกม (Host) |

## กติกาเกม

- **Civilian** (คนส่วนใหญ่): ได้คำเดียวกัน เช่น "แอปเปิล"
- **Undercover** (1 คน): ได้คำใกล้เคียง เช่น "ส้ม"
- **Mr. White** (ถ้ามี 5+ คน): ไม่ได้คำเลย — ต้องแอบทำตัวว่าคิดออก

แต่ละรอบ:
1. ทุกคนบอก **คำอธิบาย 1 คำ** เกี่ยวกับคำของตัวเอง
2. Host พิมพ์ `!uc next` เพื่อเปิดโหวต
3. โหวตคนที่คิดว่าเป็น Undercover
4. คนที่ได้โหวตมากที่สุดถูก淘汰
5. Civilian ชนะเมื่อ淘汰 Undercover ได้หมด | Undercover ชนะเมื่อเหลือคนน้อยกว่าเท่ากับ Undercover

## โครงสร้างโปรเจกต์

```
discord-undercover-bot/
├── index.js          # Main bot & commands
├── config.js         # Configuration
├── words.js          # คำคู่สำหรับเกม
├── game/
│   └── UndercoverGame.js  # Logic เกม
├── package.json
└── README.md
```

## เพิ่มคำใหม่

แก้ไข `words.js` — แต่ละแถวคือ `[คำ Civilian, คำ Undercover]`
# undercover-discord-bot
